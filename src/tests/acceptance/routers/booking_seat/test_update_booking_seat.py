...
class TestUpdateBookingSeatRouter(_TestBaseBookingSeatEndpoints):
    def test_field_prices_have_invalid_format(self):
        ...
        response = self.client.patch(url=f'{self.base_path}/{booking_seat.id}', json=payload, exp_code=422)
    ...
    def test_booking_not_found(self):
        ...
        response = self.client.patch(url=f'{self.base_path}/{booking_seat.id}', json=payload, exp_code=404)
    ...
    def test_update_invalid_booking(self, booking_factory):
        booking_seat = PendingPaymentBookingSeatFactory()
    ...
    def test_update_seat_not_available(self):
        booking_seat = PendingPaymentBookingSeatFactory()
        confirmed_booking_seat = ConfirmedBookingSeatFactory()
        booking = PendingPaymentBookingFactory(showtime=confirmed_booking_seat.booking.showtime)

        payload = {
            'booking_id': str(booking.id),
            'seat_id': confirmed_booking_seat.seat.id,
            'base_price': str(
                self.faker.pydecimal(left_digits=2, right_digits=2, positive=True, min_value=5, max_value=20)
            ),
            'vat_rate': str(
    ...
    @pytest.mark.parametrize(
        'booking_seat_factory',
        [ConfirmedBookingSeatFactory, CancelledBookingSeatFactory, ExpiredBookingSeatFactory],
        ids=['confirmed', 'cancelled', 'expired'],
    )
    def test_update_invalid_booking_seat(self, booking_seat_factory):
        booking_seat = booking_seat_factory()
        booking = PendingPaymentBookingFactory()
        seat = EnabledSeatFactory()
        payload = {
            'booking_id': str(booking.id),
    ...
    def test_update_booking_seat(self):
        booking_seat = PendingPaymentBookingSeatFactory()
        booking = PendingPaymentBookingFactory()
        seat = EnabledSeatFactory()
        ...
        response = self.client.patch(url=f'{self.base_path}/{booking_seat.id}', json=payload)
