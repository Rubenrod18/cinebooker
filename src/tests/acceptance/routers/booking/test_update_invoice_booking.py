...
from app.utils import financials, generate_unique_code
...
class TestUpdateBookingEndpoint(_TestBaseBookingEndpoints):
    ...
    def test_update_invoice_without_discounts(self):
        ...
        invoice = IssuedInvoiceFactory(booking=booking, code=generate_unique_code(), currency=Currency.EUR.value)
        InvoiceItemFactory(booking=booking, invoice=invoice)
        total_vat_price = financials.calculate_vat_amount(base_price, vat_rate)

        response = self.client.patch(url=self.endpoint(str(booking.id)), json={})
        ...
        with self.app.container.session() as session:
            ...
            assert session.query(InvoiceItem).count() == 1
    ...
    def test_update_invoice_with_discounts(self):
        base_price = Decimal('5.00')
        vat_rate = Decimal('0.21')
        price_with_vat = financials.apply_vat_rate(base_price, vat_rate)
        ...
        InvoiceItemFactory(booking=booking, invoice=invoice, description=f'Discount {discount.code}')
        ...
        with self.app.container.session() as session:
            ...
            assert session.query(InvoiceItem).count() == 2
            seat_invoice_item = session.query(InvoiceItem).filter(InvoiceItem.description == 'Seats').first()
            ...
            discount_invoice_item = (
                session.query(InvoiceItem).filter(InvoiceItem.description.like('%Discount%')).first()
            )
